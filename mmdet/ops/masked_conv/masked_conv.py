...
def masked_conv2d_jit(features, mask, weight, bias, padding=0, stride=1):
    ...
    if mask_inds.numel() > 0:
        ...
        data_col = features.new_zeros(in_channel * kernel_h * kernel_w,
                                      mask_inds.size(0))
        data_col = torch.ops.my_ops.masked_im2col_jit_forward_cuda(features, mask_h_idx,
                                                 mask_w_idx, kernel_h,
                                                 kernel_w, pad_h, pad_w,
                                                 data_col)
        ...
        masked_output = torch.addmm(1, bias[:, None], 1,
                                    weight.view(out_channel, -1), data_col)
        output = torch.ops.my_ops.masked_col2im_jit_forward_cuda(masked_output, mask_h_idx,
                                                 mask_w_idx, out_h, out_w,
                                                 out_channel, output)
    return output
