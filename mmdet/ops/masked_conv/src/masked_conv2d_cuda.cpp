...
at::Tensor masked_im2col_jit_forward_cuda( at::Tensor im,  at::Tensor mask_h_idx,
                               at::Tensor mask_w_idx,  int64_t kernel_h,
                               int64_t kernel_w,  int64_t pad_h,
                               int64_t pad_w, at::Tensor col) {
  CHECK_INPUT(im);
  CHECK_INPUT(mask_h_idx);
  CHECK_INPUT(mask_w_idx);
  CHECK_INPUT(col);
...
at::Tensor masked_col2im_jit_forward_cuda( at::Tensor col,
                                at::Tensor mask_h_idx,
                                at::Tensor mask_w_idx, int64_t height,
                               int64_t width, int64_t channels, at::Tensor im) {
  CHECK_INPUT(col);
  CHECK_INPUT(mask_h_idx);
  CHECK_INPUT(mask_w_idx);
  CHECK_INPUT(im);
