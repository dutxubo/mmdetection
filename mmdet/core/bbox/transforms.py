...
def keypoint2result(keypoints, labels, num_classes, num_keypoints=9):
    ...
    Args:
        keypoints (Tensor): shape (n, 2*k)
        labels (Tensor): shape (n, )
        num_classes (int): class number, including background class
        num_keypoints (int): number of categories of key points
    ...
    if keypoints.shape[0] == 0:
        return [
            np.zeros((0, num_keypoints), dtype=np.float32) for i in range(num_classes - 1)
        ]
    else:
        keypoints = keypoints.detach().cpu().numpy()
        labels = labels.detach().cpu().numpy()
        return [keypoints[labels == i, :] for i in range(num_classes - 1)]
