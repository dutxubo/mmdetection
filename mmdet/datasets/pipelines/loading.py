...
class LoadAnnotations(object):
    ...
    def _load_keypoints(self, results):
        ...
        ## gt_masks : [[[x1,y1,x2,y2,x3,y3,x4,y4]],[[x1,y1,x2,y2,x3,y3,x4,y4]]]
        ## gt_keypoints : [[x1,y1,x2,y2,x3,y3,x4,y4],[x1,y1,x2,y2,x3,y3,x4,y4]]   top-left, top-right, bottom-right, bottom-left     ##anticlockwise
        results['gt_keypoints'] =  [mask[0] for mask in gt_masks] 
        ##指定目标检测只有4个角点
        for i in range(len(results['gt_keypoints'])):           
            if len(results['gt_keypoints'][i]) !=8  :
                pts = np.array(results['gt_keypoints'][i]).reshape(-1,2)
                
                # the top-left point will have the smallest sum, whereas
                # the bottom-right point will have the largest sum
                s = pts.sum(axis = 1)
                x1, y1 = pts[np.argmin(s)]
                x3, y3 = pts[np.argmax(s)]
                 
                # now, compute the difference between the points, the
                # top-right point will have the smallest difference,
                # whereas the bottom-left will have the largest difference
                diff = np.diff(pts, axis = 1)
                x2, y2 = pts[np.argmin(diff)]
