...
def myimrescale(img, scale, return_scale=False, interpolation='bilinear'):
    ...
    if isinstance(scale, (float, int)):
        ...
        # mmcv.imrescale : the image will be rescaled as large as possible within the scale.
...
class Resize(object):
    ...
    Args:
        ...
        keep_ratio (bool): Whether to keep the aspect ratio when resizing the
            image.
        min_rescale (bool): when keep_ratio is True it works. min_rescale is True which 
            means the minimum scaling principle is used to ensure that the specified scale is 
            not exceeded 'img_scale', otherwise mmcv.imrescale is used.
    """
    ...
    def __init__(self,
        ...
        self.imrescale =  myimrescale if min_rescale else mmcv.imrescale
    ...
    def _resize_img(self, results):
        if self.keep_ratio:
            ...
            img, scale_factor = self.imrescale(results['img'], results['scale'], return_scale=True) 
        ...
        results['w_scale'] = w_scale
        results['h_scale'] = h_scale
    ...
    def _resize_bboxes(self, results):
        ...
        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            bboxes[:, 0::2] = np.clip(bboxes[:, 0::2] * results['w_scale'], 0, img_shape[1] - 1)
            bboxes[:, 1::2] = np.clip(bboxes[:, 1::2] * results['h_scale'], 0, img_shape[0] - 1)
            results[key] = bboxes
...
class RandomFlip(object):
    ...
    def keypoint_flip(self, keypoints, img_shape):
        ...
        Args:
            keypoints(ndarray): shape (..., 2*n*k)
            img_shape(tuple): (height, width)
        """
        assert keypoints.shape[-1] % 2 == 0
        w = img_shape[1]
        flipped = keypoints.copy()
        flipped[..., 0::2] = w - keypoints[..., 0::2] - 1
        return flipped
...
@PIPELINES.register_module
class BboxSafeRandomCrop(object):
    """Random crop the image & bboxes & keypoints. all bboxes are in crop region.

    Args:
        crop_ratio (tuple): Expected crop_ratio_range.
    """

    def __init__(self, crop_range=(0.7, 0.9) ):
    ...
    def __call__(self, results):
        ...
        max_x = int( gt_bboxes[:, 2].max() )
        max_y = int( gt_bboxes[:, 3].max() )
        
        crop_h_ratio = np.random.uniform(*self.crop_range)
        crop_w_ratio = np.random.uniform(*self.crop_range)
        
        crop_h = int( max( img.shape[0] * crop_h_ratio , max_y - min_y ) )
        crop_w = int( max( img.shape[1] * crop_w_ratio , max_x - min_x ) )
        ...
        # crop bboxes accordingly
        for key in results.get('bbox_fields', []):
            bboxes = results[key]
            bboxes[:, 0::2] = np.clip(bboxes[:, 0::2] - offset_w, 0, img_shape[1] - 1)
            bboxes[:, 1::2] = np.clip(bboxes[:, 1::2] - offset_h, 0, img_shape[0] - 1)
            results[key] = bboxes
            
        for key in results.get('keypoint_fields', []):
            keypoints = results[key] 
            keypoints[:, 0::2] = np.clip(keypoints[:, 0::2] - offset_w, 0, img_shape[1] - 1)
            keypoints[:, 1::2] = np.clip(keypoints[:, 1::2] - offset_h, 0, img_shape[0] - 1)
            results[key] = keypoints
