'''
https://github.com/lizhe960118/CenterNet/tree/master/mmdet/models/anchor_heads
'''
...
class CenterHead(nn.Module):
    ...
    def __init__(self,
                 ...
                 use_cross = False,
                 loss_hm = dict( type="CenterFocalLoss"), # 这里实现 CenterFocalLoss
    ...
    def loss(self,
        ...
        #print(num_center, pos_wh_targets)
        if num_center > 0:
    ...
    def center_target_single(self, gt_bboxes, gt_labels, img_meta):
        ...
        for k in range(num_objs):
            ...
            for index_level in index_levels:
                stride = self.strides[index_level]
                ...
                if h > 0 and w > 0:
                    ...
                    #hm[cls_id-1, ct_int[1], ct_int[0]] = 1
    ...
    def get_bboxes_single(self,
        ...
        for cls_score, wh_pred, offset_pred, featmap_size in zip(
            ...
            dets = post_process(dets, stride, output_h, output_w, scale=scale_factor, num_classes=self.cls_out_channels)
            detections.append(dets)
        
        results = merge_outputs(detections, self.cls_out_channels) # 单张图的结果
...
def post_process(dets, stride, out_height, out_width, scale, num_classes):
...
def ctdet_post_process(dets, stride, h, w, num_classes):
    ret = []
#     print(dets.shape) # (1, 100, 6)
#     print(c)
    for i in range(dets.shape[0]):
        top_preds = {}
        dets[i, :, :2] = dets[i, :, 0:2]*stride
        dets[i, :, 2:4] = dets[i, :, 2:4]*stride
