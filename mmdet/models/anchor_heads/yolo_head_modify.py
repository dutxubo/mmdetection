...
# https://github.com/open-mmlab/mmdetection/pull/1695/files#diff-ab0814f43cd90c8eaaebe477f5812b77R74
import torch
...
class YoloModifyHead(nn.Module):
    ...
    def __init__(self,
        ...
        super(YoloModifyHead, self).__init__()
        ...
        self.num_classes_w_bkg = num_classes
        self.num_classes_no_bkg = num_classes - 1
        ...
        self.hierarchical_label = loss_cls['type'] in ['HierarchicalCrossEntropyLoss']
    ...
    def forward(self, feats):
        ...
        # results :[ [ bs, num_anchor*(cxcywh+conf+class), h, w ], ]
    ...
    def get_bboxes_single(self, results_raw, scale_factor, cfg, rescale=False):
        ...
        for i_scale in range(self.num_scales):
            ...
            bbox_pred = torch.stack((x1_pred, y1_pred, x2_pred, y2_pred), dim=3).view((-1, 4))  # cxcywh
        ...
        det_bboxes, det_labels = multiclass_nms(multi_lvl_bboxes, multi_lvl_cls_scores, cfg.score_thr, cfg.nms, cfg.max_per_img, score_factors=multi_lvl_conf_scores)
    ...
    def loss(self,
        ...
        for img_id in range(len(img_metas)):
            ...
            for i_scale in range(self.num_scales):
                ...
                num_grid_h = pred_raw.size(-2)
                num_grid_w = pred_raw.size(-1)
                pred_raw = pred_raw.view(self.num_anchors_per_scale,
                                         self.num_attrib,
                                         num_grid_h,
                                         num_grid_w).permute(0, 2, 3, 1).contiguous()
                anchor_grid = self.get_anchors(num_grid_h, num_grid_w, i_scale, device=pred_raw.device) # 3*h*w*4
    ...
    def loss_single(self, preds_raw, gts_t, neg_masks, xy_use_logit=False, balance_conf=False):
        ...
        for i_scale in range(self.num_scales):
            ...
            #import pdb
            #pdb.set_trace()
            pred_pos_label = pred_label[pos_mask.bool()]
            gt_pos_label = gt_label[pos_mask.bool()].float()
            
            #losses_cls = F.binary_cross_entropy_with_logits(pred_label, gt_label, reduction='none')
            losses_cls = self.loss_cls(pred_pos_label, gt_pos_label, reduction='none')
    ...
    def _preprocess_target_single_img(self, gt_bboxes, gt_labels, anchor_grids, ignore_thresh, one_hot_smoother=0, xy_use_logit=False):
        ...
        for anchor_grid in anchor_grids:
            ...
            negative_mask = anchor_grid.new_ones(negative_mask_size, dtype=torch.uint8) # 3*h*w
            negative_mask_across_scale.append(negative_mask)
            if self.use_sigmoid_cls:
                gt_t_size = negative_mask_size + [self.num_attrib]
            else:
                gt_t_size = negative_mask_size + [5+1]
            gt_t = anchor_grid.new_zeros(gt_t_size) # 3*h*w*num_attrib
        ...
        for gt_bbox, gt_label in zip(gt_bboxes, gt_labels):
            ...
            for i_scale in range(self.num_scales):
                ...
                # reference to  https://github.com/huggingface/transformers/pull/2369
                if torch.__version__ >= '1.2.0':
                    negative_mask = (iou_gt_anchor <= ignore_thresh).byte()
                else:
            ...
            gt_tcx = (gt_cx / self.anchor_strides[match_scale] - match_grid_w).clamp(_EPSILON, 1 - _EPSILON)
            gt_tcy = (gt_cy / self.anchor_strides[match_scale] - match_grid_h).clamp(_EPSILON, 1 - _EPSILON)
    ...
    def get_anchors(self, num_grid_h, num_grid_w, scale, device='cpu'):
        ...
        stride = self.anchor_strides[scale]
        ...
        grid_y += stride / 2
        grid_x = grid_x.expand((num_anchors, -1, -1)) #3*h*w
        grid_y = grid_y.expand((num_anchors, -1, -1))

        anchor_w = anchors[:, 0:1].view((-1, 1, 1))
        anchor_h = anchors[:, 1:2].view((-1, 1, 1))
        anchor_w = anchor_w.expand((-1, num_grid_h, num_grid_w)) # 3*h*w
        anchor_h = anchor_h.expand((-1, num_grid_h, num_grid_w))

        anchor_cxywh = torch.stack((grid_x, grid_y, anchor_w, anchor_h), dim=3) # 3*h*w*4
